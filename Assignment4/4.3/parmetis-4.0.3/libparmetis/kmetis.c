  ...
  if (graph->gnvtxs < 1.3*ctrl->CoarsenTo ||
    ...
    if (graph->ncon > 1 && graph->level < 3) {
      if(ncon%2 == 1){  
        ...
        // 2-level loop unrolling
        for (i=0; i<ncon/2; i += 2) {
          ftmp = rsum(nparts, graph->gnpwgts+i, ncon);
          if (ftmp != 0.0)
            lbvec[i] = (real_t)(nparts) *
            graph->gnpwgts[rargmax_strd(nparts, graph->gnpwgts+i, ncon)*ncon+i]/ftmp;
            ...
            lbvec[i] = 1.0;
          // Added by AR
          ftmp = rsum(nparts, graph->gnpwgts+i+1, ncon);
          if (ftmp != 0.0)
            lbvec[i+1] = (real_t)(nparts) *
            graph->gnpwgts[rargmax_strd(nparts, graph->gnpwgts+i+1, ncon)*ncon+i+1]/ftmp;
          else
            lbvec[i+1] = 1.0;
        }
      }  
